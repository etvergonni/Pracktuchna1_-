...
while (running)
{
    ...
    switch (choice)
    {
        ...
        case "57": AddStudentEventHandler(); break;
        case "58": FilterPredicate(); break;
        case "59": OperationAction(); break;
        case "60": ReportFunc(); break;
        case "61": EventSystemDemo(); break;
        case "62": SortLambda(); break;
        case "63": TestCallbacks(); break;
        case "64": EventHistory(); break;
        case "65": UndoRedoDemo(); break;
        case "0":
            running = false;
...
void ShowMenu()
{
    ...
    Console.WriteLine("56. Імпорт студентів з CSV (Варіант ПР8)");
    Console.WriteLine();
    Console.WriteLine("Делегати та події (ПР9):");
    Console.WriteLine("57. Обробник події при додаванні студента");
    Console.WriteLine("58. Фільтр через Predicate (відмінники, боржники)");
    Console.WriteLine("59. Операція над студентами через Action");
    Console.WriteLine("60. Згенерувати звіт через Func");
    Console.WriteLine("61. Продемонструвати систему подій");
    Console.WriteLine("62. Сортування студентів лямбда-виразами");
    Console.WriteLine("63. Тестування callback-механізмів");
    Console.WriteLine("64. Переглянути історію подій");
    Console.WriteLine("65. Undo/Redo дій над групою (Варіант 2)");
...
void AddStudentEventHandler()
{
    group.StudentAdded += (sender, e) =>
        Console.WriteLine($"  [Подія StudentAdded] {e.Message}: {e.Student.FullName}");
    Console.WriteLine("Обробник підписано на подію StudentAdded.");

    string rec = (DateTime.Now.Ticks % 100000000).ToString("D8");
    var s = new Student("Подієвий Студент Демонстраційний", new DateTime(2005, 1, 1), rec);
    try
    {
        group.AddStudentWithEvents(s);
        Console.WriteLine("Студента додано, обробник події спрацював вище.");
    }
    catch (Exception ex)
    {
        Console.WriteLine($"  {ex.Message}");
    }
    logger.Info("Демонстрація обробника події додавання");
}

void FilterPredicate()
{
    Predicate<Student> excellent = s => s.IsExcellent();
    Predicate<Student> debtors = s => s.IsFailing();

    var ex = group.FilterStudents(excellent);
    var db = group.FilterStudents(debtors);

    Console.WriteLine($"Відмінники ({ex.Count}):");
    foreach (var s in ex) Console.WriteLine($"  {s.FullName}, бал {s.AverageGrade:F2}");

    Console.WriteLine($"Боржники ({db.Count}):");
    foreach (var s in db) Console.WriteLine($"  {s.FullName}, бал {s.AverageGrade:F2}");
    logger.Info("Фільтрація через Predicate");
}

void OperationAction()
{
    Console.WriteLine("Операція над відмінниками через Action:");
    group.PerformOperationOnStudents(
        s => s.IsExcellent(),
        s => Console.WriteLine($"  Вітаємо, {s.FullName}! Середній бал {s.AverageGrade:F2}"));
    logger.Info("Операція над студентами через Action");
}

void ReportFunc()
{
    Func<StudentGroup, string> reporter = g =>
    {
        var sb = new StringBuilder();
        sb.AppendLine($"Звіт по групі {g.GroupName}");
        sb.AppendLine($"Студентів: {g.GetAllStudents().Count}");
        sb.AppendLine($"Середній бал групи: {g.CalculateMetric(s => s.AverageGrade):F2}");
        return sb.ToString();
    };

    string report = group.GenerateReport(reporter);
    Console.WriteLine(report);
    eventManager.RaiseReportGenerated(group, report);
    logger.Info("Генерація звіту через Func");
}

void EventSystemDemo()
{
    Console.WriteLine("Демонстрація системи подій (багатовіщальний делегат):");

    EventHandler<StudentEventArgs> h1 = (s, e) =>
        Console.WriteLine($"  Підписник 1: {e.Message} ({e.Student.FullName})");
    EventHandler<StudentEventArgs> h2 = (s, e) =>
        Console.WriteLine($"  Підписник 2: запис у лог про {e.Student.FullName}");

    eventManager.StudentAdded += h1;
    eventManager.StudentAdded += h2;

    string rec = (DateTime.Now.Ticks % 100000000).ToString("D8");
    var demo = new Student("Демонстраційний Студент Подій", new DateTime(2005, 1, 1), rec);
    eventManager.RaiseStudentAdded(group, demo);

    eventManager.StudentAdded -= h1;
    eventManager.StudentAdded -= h2;
    Console.WriteLine("Обидва підписники отримали одну подію (multicast).");
    logger.Info("Демонстрація системи подій");
}

void SortLambda()
{
    Console.WriteLine("Сортування студентів за середнім балом (спадання) через лямбду:");
    var sorted = group.SortStudents((a, b) => b.AverageGrade.CompareTo(a.AverageGrade));
    foreach (var s in sorted)
        Console.WriteLine($"  {s.FullName}: {s.AverageGrade:F2}");
    logger.Info("Сортування лямбда-виразами");
}

void TestCallbacks()
{
    Console.WriteLine("Тестування callback-механізмів:");

    Action<string> onComplete = msg => Console.WriteLine($"  Callback: {msg}");
    Func<Student, string> describe = s => $"{s.FullName} (бал {s.AverageGrade:F2})";

    var students = group.GetAllStudents();
    onComplete($"знайдено {students.Count} студентів");
    if (students.Count > 0)
        onComplete(describe(students[0]));
    logger.Info("Тестування callback");
}

void EventHistory()
{
    var history = eventManager.GetHistory();
    if (history.Count == 0)
    {
        Console.WriteLine("Історія подій порожня. Скористайтесь пунктами 60 або 61.");
        return;
    }
    Console.WriteLine($"Історія подій ({history.Count}):");
    foreach (var record in history)
        Console.WriteLine($"  {record}");
}
...
void UndoRedoDemo()
{
    Console.WriteLine("Демонстрація Undo/Redo дій над групою через делегати (Варіант 2):");

    string rec = (DateTime.Now.Ticks % 100000000).ToString("D8");
    var student = new Student("Скасовуваний Студент Демонстраційний", new DateTime(2005, 1, 1), rec);

    int before = group.GetAllStudents().Count;
    undoRedo.Execute(
        $"Додавання студента {student.FullName}",
        () => group.AddStudent(student),
        () => group.RemoveStudent(rec));
    Console.WriteLine($"  Дію виконано. Було {before}, стало {group.GetAllStudents().Count}");

    undoRedo.Undo();
    Console.WriteLine($"  Undo (скасування): стало {group.GetAllStudents().Count}");

    undoRedo.Redo();
    Console.WriteLine($"  Redo (повтор): стало {group.GetAllStudents().Count}");

    Console.WriteLine($"  Стек undo: {undoRedo.UndoCount}, стек redo: {undoRedo.RedoCount}");
    logger.Info("Демонстрація Undo/Redo");
}
